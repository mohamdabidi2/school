...
const PageAbsences = async ({ searchParams }: { searchParams: { [key: string]: string | undefined } }) => {
  ...
  // Role-scoped filtering: teachers see their own absences; others default to all (admin) or none
  ...
  const columns = [
    ...
    { enTete: 'Motif', cle: 'reason', classeNom: 'hidden md:table-cell' },
    { enTete: 'Actions', cle: 'action' },
  ];

  const renderRow = (item: AbsenceRow) => (
    <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight">
      ...
      <td className="hidden md:table-cell max-w-[320px] truncate">{item.reason || '-'}</td>
      <td>
        <div className="flex items-center gap-2">
          <FormContainer table="teacherAbsence" type="update" data={item} />
          <FormContainer table="teacherAbsence" type="delete" id={item.id} />
        </div>
      </td>
    </tr>
  );
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Absences enseignants</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            <FormContainer table="teacherAbsence" type="create" />
          </div>
        </div>
      </div>
